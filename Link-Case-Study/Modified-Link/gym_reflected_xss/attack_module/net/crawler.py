...
# Conditional import to avoid Python version compatibility issues
try:
...
from gym_reflected_xss.attack_module.net import web
from gym_reflected_xss.attack_module.net import lamejs
        ...
        for form in self.soup.find_all("form"):
            ...
            for input_field in form.find_all("input", attrs={"name": True}):
                ...
                """
                    ...
                    if method == "GET":
                        get_params.append([input_field["name"], "test"])
                    else:
                        post_params.append([input_field["name"], "test"])
                """
...
class Crawler:
    ...
    def is_in_scope(self, resource):
        ...
        if self._scope == Scope.PUNK:
            ...
            try:
                print(resource)
                if "localhost" in str(resource)  or "gsiscms.kaist.ac.kr" in str(resource) or  "gsis.kaist.ac.kr" in str(resource):
                    return True
                else:
                    return False 
            ...
            if isinstance(resource, web.Request):
                #print(resource)
                if resource._hostname.split(":")[0] == "localhost": # or "gsiscms.kaist.ac.kr" in resource or  "gsis.kaist.ac.kr" in resource:
                    
                    return True
                else:
                    return False 
            else:
                if "localhost" in resource :#or "143.248.247.127" in resource:
                    return True
                else:
                    return False
        ...
        if isinstance(resource, web.Request):
            if self._scope == Scope.FOLDER:
                return resource.url.startswith("http://localhost/cms/geeklog2/public_html/admin/plugins.php")
    ...
    def login(self):
        ...
        start_url = "http://localhost/cms/seopanel/alerts.php"
        login_url = "http://localhost/cms/seopanel/login.php"

        LOGIN_INFO = {
        "sec": "login",
        "red_referer": "http://localhost/cms/seopanel/admin-panel.php",
        "userName": "spadmin",
        "password": "spadmin",
        "login":""
        }
        ...
        start_url = "http://localhost/cms/geeklog2/public_html/admin/index.php"
        login_url = "http://localhost/cms/geeklog2/public_html/admin/index.php"
        ...
        """
        """
        start_url = "http://localhost/cms/ampache3/index.php"
        login_url = "http://localhost/cms/ampache3/login.php"
        ...
        """
        header = {
            'Referer': 'http://localhost/cms/taocms/admin/admin.php?action=frame&ctrl=login'
            
        }

        login_res = self._session.post(login_url, data=LOGIN_INFO)
        #login_res = self._session.get("GET /process.php?do=login&username=test12345&password=test12345&language=en&_=1626398566333")
        print("Login!\n")
        
        #print(login_res.text)
        req = self._session.get(start_url)
        #print(req.text)
        start_url = ""
    ...
    def get(self, resource: web.Request, follow_redirects: bool = False, headers: dict = None) -> Page:
        ...
        #else: 
        #    headers["referer"] = "http://localhost/cms/taocms/admin/admin.php?action=frame&ctrl=iframes"
        try:
    ...
    def post(self, form: web.Request, follow_redirects: bool = False, headers: dict = None) -> Page:
        ...
        #else:
        #    form_headers["referer"] = "http://localhost/cms/taocms/admin/admin.php?action=frame&ctrl=iframes"
...
class Explorer:
    ...
    def explore(
        ...
        # selenium login
        #f = open('gym_reflected_xss/attack_module/net/login_urls.txt')
        
        #if start_url in f.read():

        #login_part
        #redirect_url = self._crawler.login()


        #to_explore.append(web.Request("https://localhost:8443/benchmark/xss-00/BenchmarkTest00385?BenchmarkTest00385=SafeText&foo=bar", link_depth=0))
        #to_explore.append(web.Request("http://localhost/cms/chamilo/main/calendar/agenda_list.php?type=#collapse-personal_1", link_depth=0))
        #to_explore.append(web.Request("http://localhost/cms/seopanel/alerts.php?alert_category=general&from_time=&keyword=&to_time=2021-03-11", link_depth=0))
        #to_explore.append(web.Request("http://localhost/cms/geeklog2/public_html/admin/plugins.php?direction=ASC&order=1&prevorder=pi_load", link_depth=0))
        #to_explore.append(web.Request("http://localhost/cms/geeklog/public_html/admin/trackback.php", link_depth=0))
        
        #f.close()
        #to_explore.append(web.Request("http://143.248.247.127:8021/page1.php", link_depth=0))
        #to_explore.append(web.Request("http://143.248.247.127:8009/Assets-List.php?st%5B%5D=&st%5B%5D=&st%5B%5D=&st%5B%5D=&upd=Update&del=Delete&in%5B%5D=usrname&op%5B%5D=%7C&co%5B%5D=%3C&in%5B%5D=usrname&op%5B%5D=%7C&co%5B%5D=OR&in%5B%5D=usrname&op%5B%5D=%7C&co%5B%5D=%3C&in%5B%5D=usrname&op%5B%5D=%7C&col%5B%5D=endsupport&lim=250&as=250&ms=20", link_depth=0))
        #to_explore.append(web.Request("http://143.248.247.127:8009/Assets-List.php?st%5B%5D=&st%5B%5D=&st%5B%5D=&st%5B%5D=&upd=Update&del=Delete&in%5B%5D=%22%2F%3E%3CSvG%2FoNloAd%3Dalert%28%2Fw6498vfyb8%2F%29%3E&op%5B%5D=%7C&co%5B%5D=%3C&in%5B%5D=usrname&op%5B%5D=%7C&co%5B%5D=OR&in%5B%5D=usrname&op%5B%5D=%7C&co%5B%5D=%3C&in%5B%5D=usrname&op%5B%5D=%7C&col%5B%5D=endsupport&lim=250&as=250&ms=20", link_depth=0))
        
    
        new_form = web.Request(
                "http://143.248.247.127:8033/edit/profile",
                method="P",
                get_params=[],
                post_params=[["title", "test"], ["content", "dummy"], ["cstf_token","b15f1a0dc2fa6c8916cece0fe35de1c23bdbc1d4" ],["submit", "Save"]],
                file_params=[],
                encoding="utf-8",
                referer="http://143.248.247.127:8033/edit/profile",
                enctype="application/x-www-form-urlencoded"
            )
        #to_explore.append(new_form)
        while to_explore:
            ...
            try:
                ...
                print(("[!] SSL/TLS error occurred with URL"), resource_url)
                continue
            # TODO: what to do of connection errors ? sleep a while before retrying ?
            except ConnectionError:
                print(("[!] Connection error with URL"), resource_url)
                continue
            except RequestException as error:
                print(("[!] {} with url {}").format(error.__class__.__name__, resource_url))
                continue
            ...
            allowed_links = []
            # print(page.links)
            if page.type.startswith(MIME_TEXT_TYPES):
